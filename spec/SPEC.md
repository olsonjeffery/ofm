...
| **✅ Yes** | [`extra/system-status-and-health.md`](./extra/system-status-and-health.md) | System Status & Health monitoring: a code-based bin dependency check (required set blocks startup), live sub-system probes (opencode pool, rauthy/OIDC, hiqlite, gh), the rolling `system-health-entry` table, a `/webapp/system` page + navbar running-services badge + startup console report, the `system-status` capability (OAuth-scope group) gating agent-session injection, and the `ofm health` CLI (`--teardown`/`--do-teardown <PID>`, `--global`, `--global --do-teardown`; exit codes 0/1/2/3; `{footprint}/ofm.pid` marker; restart guard; precise owned-group/exact-PID/named-container teardown). See `src/services/system_health.rs`, `src/procscan.rs`, `src/cli.rs`. |
