...
#[tokio::test]
async fn test_import_execute_create_new() {
    let (state, auth_layer, api_key, tmp) = make_state_with_auth().await;
    let db = state.db.clone();
    let user_id = state.default_user_id;
    let archive_root = tmp.path().to_str().unwrap().to_string();
    ...
    let raw_json = make_export_json(42);
    ...
    let resp = client
        ...
        .json(&serde_json::json!({
            ...
            "imports": [{
                ...
                "target_type": "create_new",
                "name": "Imported Project",
                "repo_folder_path": "/tmp/imported"
            }]
        }))
        .send()
        .await
    ...
    assert_eq!(
        resp.status(),
        200,
        "import execute should succeed: {:?}",
        resp.text().await
    );

    // Verify project was created
    let projects = db
        .query_map::<ofm::db::schema::Project, _>(
            "SELECT id, user_id, name, repo_folder_path, subproject_path, created_at \
             FROM projects WHERE user_id = $1 ORDER BY id DESC LIMIT 1",
            hiqlite::params!(user_id.to_string()),
        )
        .await
        .unwrap();
    let project = projects.first().expect("should have at least one project");
    assert_eq!(project.name, "Imported Project");
    assert_eq!(project.repo_folder_path, "/tmp/imported");

    // Verify task was created with new ID
    let tasks = db
        .query_map::<ofm::db::schema::Task, _>(
            ...
            hiqlite::params!(project.id),
        )
        .await
        .unwrap();
    assert_eq!(tasks.len(), 1);
    assert_eq!(tasks[0].title, "Test task 1");
    assert_ne!(tasks[0].id, 100, "task should have new ID, not source ID");

    // Verify task doc was written
    let doc_path = std::path::Path::new(&archive_root)
        .join("projects")
        .join(project.id.to_string())
        .join("tasks")
        .join(format!("task-{}.md", tasks[0].id));
    assert!(doc_path.exists(), "task doc should exist at {:?}", doc_path);
    let doc_content = std::fs::read_to_string(&doc_path).unwrap();
    assert_eq!(doc_content, "Task description 1");
}

#[tokio::test]
async fn test_import_execute_add_to_existing() {
    let (state, auth_layer, api_key, tmp) = make_state_with_auth().await;
    let db = state.db.clone();
    let user_id = state.default_user_id;
    let archive_root = tmp.path().to_str().unwrap().to_string();
    ...
    let existing_project_id = seed_project(&db, &user_id, &archive_root, "existing", &[]).await;
    ...
    assert_eq!(resp.status(), 200, "import execute should succeed");

    // Verify task was added to existing project
    let tasks = db
        .query_map::<ofm::db::schema::Task, _>(
            "SELECT id, project_id, user_id, title, status, workflow_complete, \
             workflow_blocked, workflow_run_count, planification_complete, \
             pr_agent_complete, refinement_complete, yolo_mode, created_at \
             FROM tasks WHERE project_id = $1",
            hiqlite::params!(existing_project_id),
        )
        .await
        .unwrap();
    assert_eq!(tasks.len(), 1, "existing project should now have 1 task");
    assert_eq!(tasks[0].title, "Test task 1");

    // Verify task doc was written for the task in existing project
    let doc_path = std::path::Path::new(&archive_root)
        .join("projects")
        .join(existing_project_id.to_string())
        .join("tasks")
        .join(format!("task-{}.md", tasks[0].id));
    assert!(doc_path.exists(), "task doc should exist");
    let doc_content = std::fs::read_to_string(&doc_path).unwrap();
    assert_eq!(doc_content, "Task description 1");
}
...
#[tokio::test]
async fn test_import_execute_create_new_with_conversations() {
    let (state, auth_layer, api_key, tmp) = make_state_with_auth().await;
    let db = state.db.clone();
    let _user_id = state.default_user_id;
    let _archive_root = tmp.path().to_str().unwrap().to_string();
    let base_url = spawn_app(state, auth_layer).await;
    ...
    let resp = client
        ...
        .json(&serde_json::json!({
            ...
            "imports": [{
                ...
                "repo_folder_path": "/tmp/conv-test"
            }]
        }))
        .send()
        .await
    ...
    let projects = db
        .query_map::<ofm::db::schema::Project, _>(
            "SELECT id, user_id, name, repo_folder_path, subproject_path, created_at \
             FROM projects WHERE name = 'Conversation Test'",
            hiqlite::params!(),
        )
        .await
        .unwrap();
    let project = projects.first().expect("project should exist");
    ...
    assert_eq!(conv.effort, "high");
    assert_eq!(conv.name.as_deref(), Some("Initial chat"));
    assert_eq!(conv.provider_session_id.as_deref(), Some("sess-001"));

    // Verify no messages exist for this conversation
    // Since messages use a different key pattern, we just verify conversation exists
    // The spec says no messages rows are created
}
