...
pub fn ChatInput(
    ...
    view! {
        <form id="chat-form" data-task-id={task_id.to_string()}
...
mod tests {
    ...
    #[test]
    fn test_chat_input_send_button_is_small() {
        let html = leptos::view! {
            <ChatInput
                _on_send=Callback::new(|_: String| {})
                disabled=false
        ...
        assert!(
            ...
            "Send button should have is-small"
        );
        assert!(
            !html.contains("is-medium"),
            "Send button should not have is-medium"
        );
    }
